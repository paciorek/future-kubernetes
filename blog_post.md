...
[here](https://github.com/paciorek/future-kubernetes) for details on
...
troubleshooting tips can be found [here](https://github.com/paciorek/future-kubernetes).
future-kubernetes). 
...
3. Connect to the RStudio running on the cluster from your browser
...
We use the Kubernetes package manager, Helm, to run the pods of interest:
 ...
 - one (server) pod for a main process that runs RStudio and
 ...
 connect to that port, allowing the server pod RStudio process to
...
Each pod runs a Docker container. I use [my own container](https://github.com/paciorek/future-kubernetes-docker) that layers a bit on top of the [Rocker](https://rocker-project.org) container that contains R and RStudio.
...
details can be found [here](https://github.com/paciorek/future-kubernetes).
...
Since the RStudio process that you interact with wouldn't generally be doing heavy
...
# Step 2: Connect to RStudio running in the cluster
...
You can now connect from your browser to the RStudio instance by entering this URL:
...
just act as if RStudio is accessible directly on your laptop. 

Note that there is nothing magical about running your computation via RStudio. You could connect to
the main pod and simply run R in it and then use the future package. 
...
from the future package don't try to start R processes on the workers, as those R processes have already been started by Kubernetes and are waiting to connect to the main (RStudio) process. 
...
output <- future_sapply(1:40, function(i) mean(rnorm(1e7)), future.seed = 1)
```

Note that all of this assumes you're working interactively, but you can always reconnect to the RStudio instance after closing the browser, and any long-running code should continue running even if you close the browser.
...
To add additional R packages, go into the `future-helm-chart`
 directory (which you created using the directions above in step 1b) and edit the `values.yaml` file. Simply modify the lines that look like
 this:
...
pod (where RStudio runs) and on the workers. If so, make sure to modify the lines above in both the `scheduler` and `worker` stanzas.
...
```{r}
library(future.apply)
future_sapply(seq_length(num_workers), function(i) system('hostname', intern = TRUE))
```
...
I've provided many of the details of how it works
[here](https://github.com/paciorek/future-kubernetes).
...
Briefly:
 ...
 2. the RStudio main process and the workers use socket connections
   (via the R function `socketConnection` to communicate:
     ...
     main pod
     - when you run `makeClusterPSOCK` in RStudio, the RStudio process attempts to
...
If you're interested in extending or improving this or collaborating
in some fashion, please feel free to get in touch with me.
